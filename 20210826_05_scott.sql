...
--------------------------------------------------------------------------------
--@ 여기까지 SOCTT 뛰어놓을 기본 놀이터는 만들어 놓음
...
--○ Table TBL_EXAMPLE1과 Table TBL_EXAMPLE2 테이블이
--   어떤 테이블스페이스에 저장되어 있는지 조회
SELECT TABLE_NAME, TABLESPACE_NAME
FROM USER_TABLES;
--==>>
...
=================================================*/
--@ SELECT 문이 내부적으로 이순서로 처리된다는거 꼭 외우기!
--@ 야매지만.... 
-- FROM WHERE?(어디서요?) GROUP BY?(그룹을 지어서요?) /
-- HAVING(가졌대요) SELECT ORDER BY(선택된 오토바이를!)
...
--==>>
/*
7369	SMITH	CLERK	7902	8   0/12/17	    800		        20
...
/* /@ (3) -> 얘는 숫자 개수가 아니라 바이트 수임. 그치만 어차피 숫자 갯수와 똑같은 결과임
...
--@ N 안붙은 애들은 바이트 수, N 붙은 애들은 글자 수
--@ 그냥 CHAR는 고정형, VAR 붙은 애들은 가변형 문자열
/*
2. 문자형 CHAR(200)        - 고정형 크기
          CHAR(10)  ← '강의실'         6Byte 이지만 10Byte 를 소모
          CHAR(10)  ← '졸린서승균'    10Byte
          CHAR(10)  ← 'OH집중서승균'  10Byte 를 초과하므로 입력 불가

          VARCHAR2         - 가변형 크기
          VARCHAR2(10) ← '강의실'        6Byte
          VARCHAR2(10) ← '잠깬서승균'   10Byte
          VARCHAR2(10) ← 'OH잠깬서승균' 10Byte 를 초과하므로 입력 불가
...
--==>> Session이(가) 변경되었습니다.
--@ SQL Developer 재시작하면 초기화됨! 쓸 때마다 지정해줘야 하는 개념
...
SELECT EMPNO, ENAME, JOB, SAL, DEPTNO
FROM EMP
WHERE DEPTNO = 20; --@ '=' 하나가 동등함을 나타내는 관계연산자
--==>>
/*
