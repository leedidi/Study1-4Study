...
( NO        NUMBER(10)      --@ 숫자형식으로 10자리까지 입력 가능(자리수)
, NAME      VARCHAR2(30)    --@ 문자형식으로 30글자까지 입력 가능
);
--==>> 에러 발생
/* --@ 권한 불충분 에러
...
*/
--> 현재 ldy 계정은 CREATE SESSION 권한만 갖고 있으며
...
*/
--@ 집 지을수 있는 권한은 받아왔는데, 집 지을 땅을 할당받지 못한 것...
--@ 우리반, 자유롭게 쓸수있는 웹하드 만듬. 전체가 1GB, 반 인원 A가 동물의 세계 동영상 1TB 받아놓음
--@  그러면 반 인원 전체가 웹하드 쓰지 못함.. 각자 얼만큼 쓰자, 이게 할당량. 테이블 맨 처음 만들었을때
--@  테이블스페이스 얼만큼 쓰라는 할당량이 정해져 있지 않음. 그러므로 사용할 수 없음...

--> 테이블 생성 권한까지 부여받은 상황이지만
--  ldy 사용자 계정의 기본 테이블스페이스(DEFAULT TABLESPACE)는
